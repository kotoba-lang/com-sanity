(ns sanity.main
  "Kotodama WASM entrypoint for the Sanity clean-room actor (L4) — Clojure port.
  ...
  facts (`sanity.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sanity")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sanity.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sanity-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sanity-compat/ready)
